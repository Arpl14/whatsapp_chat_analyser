...
def network_analysis(df):
    ...
    # Draw the graph with custom settings
    edge_colors = [mcolors.to_rgba(plt.cm.seismic(weight / max(edge_weights))[:3]) for weight in edge_weights]
    ...
    nx.draw(G, with_labels=True, node_size=node_size, node_color=node_color, 
            ...
            edge_color=edge_colors, width=3, edge_cmap=plt.cm.seismic, 
